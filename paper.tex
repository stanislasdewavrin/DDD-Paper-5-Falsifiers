...
GR/Newtonian gravity describes weak-field tests with extraordinary precision, but several DDD-specific structural features (Yukawa scale from self-consistent feedback, strong-field deflection coefficient $C_{\rm DDD}$, ultra-relativistic Lorentz deviations, bandwidth-horizon phenomenology) are not predicted at all by GR or by Newton, and constitute new falsifiable signatures of the discrete substrate. \emph{This paper} compiles four such falsifiers: (i) Yukawa modification with length scale fixed by the lattice; (ii) strong-field deflection coefficient $C_{\rm DDD} \approx 2.34$ versus PPN $C_{\rm PPN} \approx 1.47$; (iii) ultra-relativistic kinematic deviations growing with $\gamma$ from the Floquet lattice cutoff; (iv) compact-object phenomenology near the bandwidth horizon $r = \beta$. \textbf{Leverage:} the analogue earns its existence by predicting structurally distinct, currently testable signatures GR cannot offer; each is mapped to an experimental envelope that would falsify it.
}}
\bigskip


\paragraph{Programme positioning and analogue framing.}
DDD does not contest GR or Newton — they are empirical anchors. This paper does not aim to falsify GR/Newton; it aims to identify quantitative regimes where DDD makes \emph{distinct} predictions, and to map which experiments would falsify those predictions. The signal of the analogue is not whether it reproduces GR (Papers~II--IV showed it does); it is whether it predicts \emph{new} structural features that GR/Newton cannot.
...
\begin{itemize}
\item \textbf{Mystery~\#2 + \#5 (gravity, inertia, beyond weak-field).} Where the GR/Newton pair are silent (Yukawa scale; strong-field deflection coefficient; ultra-relativistic deviations; horizon physics), the discrete substrate has structural answers tied to lattice parameters. We list each prediction with its experimental envelope.
\item \textbf{Mystery~\#4 (BH singularity).} The bandwidth-horizon at $r = \beta$ is the analogue's regularisation; this paper compiles its qualitative phenomenological signatures (deferred to a future strong-field paper for full treatment).
\end{itemize}


\paragraph{Distinctive contribution and ontological reading.}
GR/Newton describe weak-field gravity precisely but \emph{cannot make further structurally-grounded predictions in regimes where their own assumptions break down} (strong-field, sub-mm, ultra-relativistic, near-horizon). This paper's distinctive contribution is to show that the same substrate that reproduced GR's weak-field results in Papers~II--IV \emph{also} produces specific deviation patterns in those exact regimes. The deviations are not free parameters tuned to data --- they follow from the lattice's structural ingredients (rate-limiter near horizon, finite per-tick bandwidth at high $\gamma$, self-consistent feedback at sub-mm scale) introduced in Papers~I--IV. The leverage is twofold: (a) the paper turns a known difficulty of GR/Newton (silence in those regimes) into a falsifiable analogue prediction; (b) the predictions cost no new fields or parameters beyond those already labelled in Papers~I--IV's hypotheses.
...
\midrule
$C_{\rm DDD} \approx 2.34$ (strong-field deflection) & derived & from Paper~IV's eikonal $\eta=2$ + lattice corrections; testable by close-passage lensing \\
Yukawa length $\lambda_Y$ & calibrated & sub-mm Eöt-Wash inverse-square-law tests; consistency window stated in Sec.~3 \\
high-$\gamma$ Lorentz-deviation function & derived & from Paper~III's Floquet quartic correction (Eq.~21 of Paper~III) \\
horizon phenomenology & qualitative & inherited from Paper~I rate-limiter + Paper~II bandwidth profile; full treatment in future strong-field paper \\
(H-feedback) self-consistent scalar back-reaction & hypothesis (newly labelled in this paper) & not derived; future origin in coupled scalar dynamics \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
\item \emph{Configuration:} four explicit predictive scenarios are evaluated as configurations of the existing rule (sub-mm sources, near-horizon orbits, ultra-relativistic packets, non-eikonal compact-object regime); the rule itself is not modified.
\item \emph{Hypotheses introduced:} (H-feedback) is the only new structural hypothesis; it is labelled here rather than left implicit, and listed in the calibration ledger.
\end{itemize}

\noindent\emph{No-epicycle statement.} No new fields are introduced on nodes or links beyond those of Papers~I--IV. (H-feedback) is the only new labelled hypothesis. No principle of (L1)--(L6) is relaxed. The paper extends the analogue by \emph{exposing predictions in new regimes}, not by accreting structure. Each falsifier is locatable in the configuration locus or in a previously-introduced hypothesis.
...
This paper does not contest GR or Newton; both remain the
empirical anchors. What the paper offers, and what GR/Newton
cannot offer, is a list of four \emph{structurally distinct
falsifiers} (Yukawa, $C_{\rm DDD}$, ultra-relativistic
deviations, horizon phenomenology) that follow from the
discrete substrate's existing ingredients without adding new
fields. The leverage is not numerical novelty in the weak-field
regime (where DDD reproduces GR); it is predictive content in
the regimes where GR/Newton are silent. Any deviation between
DDD and the empirical theories at the level of measured tests
falsifies DDD, not GR.
