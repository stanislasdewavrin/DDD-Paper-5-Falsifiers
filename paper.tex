...
the spatial-bandwidth coupling $\eta = 2$). Here we list the regimes in
which DDD makes \emph{distinct} predictions, quantify the deviations
from Newton/GR they imply, and identify the experimental envelopes that
would falsify them. The four predictions surveyed are:
...
The four windows are operationally distinct: each probes a different structural assumption of DDD, and each can be tested separately. A positive detection of any one would constitute a positive test of DDD; a positive exclusion of any one (with $\alpha_Y \sim 1$, the $\eta = 2$ ansatz, the natural lattice scale) would force a structural revision of the corresponding sector.
